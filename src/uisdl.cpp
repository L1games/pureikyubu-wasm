...
static GLuint emu_video_texture = 0;
...
extern GLuint emu_video_texture;
...
struct SelectorFile
{
	...
	ImTextureID banner = nullptr;       // Banner texture (Dvd only)
};
...
public:
	...
	void clear()
	{
		for (auto& file : files)
		{
			if (file->banner)
			{
				GLuint tex = (GLuint)(intptr_t)file->banner;
...
static ImTextureID make_banner_texture(const uint8_t* image)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, DVD_BANNER_WIDTH, DVD_BANNER_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels.data());
...
void ui_frame_video_render() {
    static std::vector<uint32_t> rgba_buffer;
    int pixelCount = xfb_width * xfb_height;
    rgba_buffer.resize(pixelCount);

    // video_buffer ya es RGB de 32 bits/pixel, decodificado por VideoInterface::YUVBlit()
    // (vi.cpp). Su orden en memoria es B,G,R,X (ver el comentario de yuv2rs/yuv2gs/yuv2bs
    // en vi.cpp) — solo hay que reordenar a R,G,B,A para que coincida con GL_RGBA.
    const uint8_t* src = (const uint8_t*)video_buffer;
    ...
    for (int i = 0; i < pixelCount; i++)
    {
        ...
        rgba_buffer[i] = (255u << 24) | ((uint32_t)b << 16) | ((uint32_t)g << 8) | (uint32_t)r;
    }

glBindTexture(GL_TEXTURE_2D, emu_video_texture);
glPixelStorei(GL_UNPACK_ALIGNMENT, 4); // Alineación estándar de 32 bits
glPixelStorei(GL_UNPACK_ROW_LENGTH, xfb_width);
...
#ifdef __EMSCRIPTEN__
EM_BOOL on_canvas_resize(int eventType, const EmscriptenUiEvent* e, void* userData)
{
    double cssWidth = 0, cssHeight = 0;
    emscripten_get_element_css_size("#canvas", &cssWidth, &cssHeight);
    if (cssWidth > 0 && cssHeight > 0)
    {
        SDL_SetWindowSize(window, (int)cssWidth, (int)cssHeight);
    }
    return EM_TRUE;
}
#endif
...
static int ui_main()
{
    ...
    if (cssWidth > 0 && cssHeight > 0)
    {
      winWidth = (int)cssWidth;
      winHeight = (int)cssHeight;
    }
    #endif

    window = SDL_CreateWindow(APPNAME_A, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, winWidth, winHeight, window_flags);
	...
	#ifdef __EMSCRIPTEN__
    emscripten_set_resize_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, nullptr, EM_FALSE, on_canvas_resize);
    #endif
...
ImGui_ImplOpenGL3_Init("#version 300 es");
	...
	mainWindowID = SDL_GetWindowID(window);
#ifdef __EMSCRIPTEN__
    emscripten_set_main_loop(ui_frame, 0, true);
#else
    while (ui_active) {
      ui_frame();
    }
...
static void ui_frame()
{
    ...
    while (SDL_PollEvent(&event)) {
		...
		if (Debug::debugger != nullptr)
{
    Debug::debugger->ProcessSdlEvent(event);
}
	...
	if (emu_running && video_buffer != nullptr && xfb_frame_ready) {
        if (emu_video_texture == 0) {
    glGenTextures(1, &emu_video_texture);
    glBindTexture(GL_TEXTURE_2D, emu_video_texture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
}
        
        // Usa la función ya existente para manejar correctamente el formato GL_RGB
        ui_frame_video_render(); 
        
        xfb_frame_ready = false;
    }

    if (emu_running && emu_video_texture != 0) {
        ImVec2 display_size = ImGui::GetIO().DisplaySize;
        ImGui::GetBackgroundDrawList()->AddImage((void*)(intptr_t)emu_video_texture, ImVec2(0, 0), display_size);
    }
