...
namespace Flipper
{
	...
	size_t FifoProcessor::GetSize()
    {
      if (!allocated)
      {
          return (readPtr <= writePtr) ? (writePtr - readPtr) : 0;
      }
			...
			default:
{
    ...
    {
        char dump[128] = {0};
        size_t base = (readPtr >= 8) ? readPtr - 8 : 0;
        for (size_t i = 0; i < 16 && (base + i) < fifoSize; i++)
        {
            char b[4];
            snprintf(b, sizeof(b), "%02X ", fifo[base + i]);
            strcat(dump, b);
        }
        Report(Channel::GP, "GFX: bytes alrededor de readPtr: %s\n", dump);
    }
	...
	uint8_t FifoProcessor::Read8()
{
    if (GetSize() < 1)
    {
        Report(Channel::GP, "GFX: FIFO/DL agotado (0)\n");
        return 0;
    }

    lock.Lock();
    uint8_t value = fifo[readPtr++];
    if (allocated && readPtr >= fifoSize)
    {
        readPtr = 0;
    }
    lock.Unlock();
