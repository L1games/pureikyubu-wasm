	...
	bool GFXCore::GL_OpenSubsystem()
{
    ...
    backend_started = true;
    Report(Channel::GP, "GFX: backend OpenGL started (context created)\n");
    return true;
#endif // GFX_NULL
}
	...
	void GFXCore::GL_BeginFrame()
	{
		...
		else
		{
			...
			glClearDepthf((float)(pe->pe.copy_clear_z.value / 16777215.0f));
	...
	void GFXCore::GL_EndFrame()
{
    ...
    Report(Channel::GP, "GFX: frame %d presented\n", gfx_frame_counter);
    frameReady = false;
    pe->frames++;
    gfx_frame_counter++;
