...
{
    Report(Channel::PI, "PI: acceso HW no mapeado, R16 %08X (se devuelve 0x0000) [pc:%08X]\n",
        addr, Core->regs.pc);
    *reg = 0;
}
...
{
    Report(Channel::PI, "PI: acceso HW no mapeado, W16 %08X = %04X [pc:%08X]\n",
        addr, (uint16_t)data, Core->regs.pc);
}
