...
static bool SdlEventToCuiKeypress(SDL_Event& event, const char*& Text, CuiVkey& Vkey, bool& shift, bool& ctrl)
{
    ...
    if (event.type == SDL_KEYDOWN) {
        ...
        if (event.key.keysym.sym >= ' ' && event.key.keysym.sym < 127) {
            ascii_buf[0] = (char)event.key.keysym.sym;
            if (shift) ascii_buf[0] = (char)toupper(ascii_buf[0]);
            Text = ascii_buf;
            return true;
        }
	...
	void Cui::DrawInternal()
{
	...
	// --- AÑADE ESTO: Crea una ventana contenedora real ---
	...
	// Banderas para habilitar el scroll en ambas direcciones ---
	ImGuiWindowFlags child_flags = ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_AlwaysVerticalScrollbar;
	
	if (ImGui::BeginChild("frontBuf", ImVec2(0, 0), false, child_flags))
	{
...
bool Cui::ProcessSdlEvent(SDL_Event& event)
{
    const char* text;
    CuiVkey cui_vk;
    ...
    OnKeyPress(text, cui_vk, shiftPressed, ctrlPressed);
	...
	void Cui::OnKeyPress(const char* Text, CuiVkey Vkey, bool shift, bool ctrl)
{
    ...
    if (Text && Text[0] != '\0') {
        if (activeWnd != nullptr) {
            ...
            activeWnd->PutChar(CuiColor::Black, CuiColor::White, cursor_x, cursor_y, wText[0]);

            cursor_x++;

            if (cursor_x >= activeWnd->width) {
                cursor_x = 0;
                cursor_y++;
            }
    ...
    if (Vkey == CuiVkey::Backspace) {
        if (cursor_x > 0) {
            cursor_x--;
            if (activeWnd != nullptr) {
                activeWnd->PutChar(CuiColor::Black, CuiColor::White, cursor_x, cursor_y, L' ');
                activeWnd->Invalidate();
            }
        }
    }
